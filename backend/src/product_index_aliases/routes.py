...
@router.get("/", response_model=list[ProductIndexAliasListResponse], status_code=status.HTTP_200_OK, summary="List all product index aliases")
async def get_product_index_aliases(service: ServiceDependency, skip: int = Query(0, ge=0, description="Number of items to skip"), limit: int = Query(100, ge=1, le=100, description="Max number of items to return")):
...
async def get_product_index_alias(alias_id: int, service: ServiceDependency):
...
async def create_product_index_alias(data: ProductIndexAliasCreate, service: ServiceDependency):
...
async def update_product_index_alias(alias_id: int, data: ProductIndexAliasUpdate, service: ServiceDependency):
...
async def delete_product_index_alias(alias_id: int, service: ServiceDependency):
