...
@router.get("/", response_model=list[TelegramMessageListResponse], status_code=status.HTTP_200_OK, summary="List all telegram messages")
async def get_telegram_messages(service: ServiceDependency, skip: int = Query(0, ge=0, description="Number of items to skip"), limit: int = Query(100, ge=1, le=100, description="Max number of items to return")):
...
async def get_telegram_message(message_id: int, service: ServiceDependency):
...
async def create_telegram_message(data: TelegramMessageCreate, service: ServiceDependency):
...
async def update_telegram_message(message_id: int, data: TelegramMessageUpdate, service: ServiceDependency):
...
async def delete_telegram_message(message_id: int, service: ServiceDependency):
