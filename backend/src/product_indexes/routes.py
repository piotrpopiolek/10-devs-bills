...
async def get_product_indexes(user: CurrentUser, service: ServiceDependency, skip: int = Query(0, ge=0, description="Number of items to skip"), limit: int = Query(100, ge=1, le=100, description="Max number of items to return")):
    """
...
async def get_product_index(product_index_id: int, user: CurrentUser, service: ServiceDependency):
    """
...
async def create_product_index(data: ProductIndexCreate, user: CurrentUser, service: ServiceDependency):
    """
...
async def update_product_index(product_index_id: int, data: ProductIndexUpdate, user: CurrentUser, service: ServiceDependency):
    """
...
async def delete_product_index(product_index_id: int, user: CurrentUser, service: ServiceDependency):
    """
